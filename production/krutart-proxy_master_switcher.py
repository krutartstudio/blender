bl_info = {
    ...
    "version": (0, 6, 0), 
    "blender": (4, 0, 0), 
    "location": "View3D > Sidebar (N-Panel) > Asset Switcher & Outliner Context Menu",
...
def outliner_context_menu_func(self, context):
    """
    Appends the 'Swap Asset' button to the Outliner context menu.
    Performs a safe, lightweight check against the cached asset list
    to ensure zero performance impact.
    """
    # 1. Safety Check: Must have an active object
    obj = context.active_object
    if not obj:
        return

    # 2. Performance Guard: Check if the object is in our CACHED list.
    # We do NOT run heavy analysis here. If it's not in the list (hit Refresh first!),
    # we don't show the button.
    prefs = get_addon_prefs(context)
    if not prefs:
        return
        
    # CollectionProperty.get(key) is a fast O(1) lookup if the collection key is 'name'
    if not prefs.swappable_assets.get(obj.name):
        return

    # 3. Determine Label (Lightweight String Check)
    # We do a quick string parse just to label the button correctly.
    target_suffix = "Version"
    name_lower = obj.name.lower()
    
    if name_lower.endswith("-p"):
         target_suffix = "-M"
    elif "-p" in name_lower: # Loose check
         target_suffix = "-M"
    elif name_lower.endswith("-m"):
         target_suffix = "-P"
...
def register():
    ...
    try:
        ...
        debug_log(f"Failed to append to Outliner menu: {e}")

def unregister():
    debug_log("Unregistering Addon")
    
    # Remove from Outliner Context Menu
    try:
        bpy.types.OUTLINER_MT_object.remove(outliner_context_menu_func)
    except Exception as e:
        debug_log(f"Failed to remove from Outliner menu: {e}")
