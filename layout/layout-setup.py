...
def get_or_create_collection(name, parent_collection, color_tag=None):
    """
    Checks if a collection with the given name exists anywhere in the blend file.
    If it exists, it's linked to the parent if not already a child.
    If it doesn't exist, it's created and linked.
    Applies a color tag if one is provided.
    ...
    """
    created = False
    # Check if the collection exists globally in the blend file.
    collection = bpy.data.collections.get(name)

    if collection is None:
        # Collection does not exist anywhere, so create it.
        collection = bpy.data.collections.new(name)
        # Link the new collection to the parent.
        parent_collection.children.link(collection)
        created = True
    else:
        # Collection already exists globally.
        # Check if it's already a child of the intended parent.
        if name not in parent_collection.children:
            # If not, link the existing collection to the parent.
            parent_collection.children.link(collection)
