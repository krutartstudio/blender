bl_info = {
    "name": "Krutart Proxy-Master switch",
    "author": "iori, krutart, gemini",
    "version": (2, 2, 0),
    "blender": (4, 2, 0),
    "location": "3D View > Sidebar > Asset Lister",
    "description": "Recursively lists linked assets and all available assets/collections from source files.",
    "warning": "",
    "doc_url": "",
    "category": "Object",
}
...
class ASSETLISTER_PG_asset_item(bpy.types.PropertyGroup):
    """Property group to hold a single asset's details (name and category)."""
...
class ASSETLISTER_OT_open_file(bpy.types.Operator):
    ...
    ## CHANGELOG: This operator now opens the file in a new Blender instance
    # using the command line, instead of relying on the default OS handler.
    def execute(self, context):
        ...
        try:
            ...
            # This is cross-platform as it uses Blender's own binary path.
...
class ListAssetsOperator(bpy.types.Operator):
    ...
    ## CHANGELOG: Updated background script to find linked libraries for recursion
    # and to provide its own logs for better debugging.
    background_script = """
import bpy
import sys
import os
...
try:
    ...
    output_data = {
        'collections': [],
        'assets': [],
        'linked_libraries': [] # New field for recursion
    }
    ...
    for lib in bpy.data.libraries:
        if lib.filepath:
            # bpy.path.abspath resolves the path relative to the current blend file
    ...
    # 4. Find all assets in the file that match the suffixes
    found_assets = []
    ...
    # 5. Print the final dictionary as a string to stdout.
    # We wrap the data and logs in a parent dictionary.
    print(repr({'logs': log_messages, 'data': output_data}))
    sys.stdout.flush()
"""

    ## CHANGELOG: Enhanced logging and error handling. It now processes the new log
    # and data structure from the background script.
    def get_remote_file_data(self, filepath):
        """Runs Blender in the background to execute our script and returns the parsed data."""
        script_file = None
        log.info(f"Preparing to run background script for: {filepath}")
        try:
    ...
    ## CHANGELOG: Reworked the entire execute method to perform a recursive scan.
    # It now correctly finds all nested linked files and their collections.
    # Added explicit logging for found collections.
    def execute(self, context):
        ...
        # A queue for files to process and a set to prevent re-scanning
        ...
        while queue:
            ...
            if remote_info and 'data' in remote_info:
                ...
                log.info(f"Successfully parsed data for {normalized_path}.")
                
                if normalized_path not in grouped_data:
                    grouped_data[normalized_path] = {'linked_assets': [], 'remote_assets': [], 'collections': []}

                grouped_data[normalized_path]['remote_assets'].extend(remote_data.get('assets', []))
                
                # Log and store collection names
                found_collections = remote_data.get('collections', [])
                if found_collections:
                    log.info(f"Found {len(found_collections)} collections in {os.path.basename(normalized_path)}: {', '.join(found_collections)}")
                grouped_data[normalized_path]['collections'].extend(found_collections)
                ...
                log.error(f"Failed to retrieve or parse data for {normalized_path}. It will be skipped.")
...
class AssetListerPanel(bpy.types.Panel):
    ...
    def draw_asset_list(self, layout, assets):
        ...
        for asset in assets:
            ...
            asset_box.label(text=f"  \u2022 {asset.name}")
    ...
    def draw(self, context):
        ...
        for group in props.file_groups:
            ...
            ## CHANGELOG: Simplified header drawing, removed 'is_internal' check.
